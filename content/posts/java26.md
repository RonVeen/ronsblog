...
This feature issues warnings when deep reflection is used to modify `final` fields, aiming to restore their integrity and enable JVM optimizations like constant folding. 
It prepares developers for a future release where such modifications will be blocked by default to prevent undefined behavior.
<br>
...
This enhancement allows the Project Leyden AOT cache to support all garbage collectors, including ZGC, by introducing a "GC-agnostic" format using logical indices. 
Previously, caches were tied to specific GCs due to memory layout differences, forcing a tradeoff between fast startup and low latency. 
The JVM now streams objects into memory and materializes them according to the active collector's requirements.
<br>
