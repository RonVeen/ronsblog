---
title: "Java 26 - part 1: JEP 500 Prepare Final Mean Final"
date: 2025-12-29
draft: false
...
Now, this works for a number of reasons. The most important one is that we use `new String("Original Message")`. If not, the compiler would have replaced every mention of `message` with `"Original Message"`, inlining the value.
...
## Enter JEP 500
With JEP-500, final finally(!) becomes final again.
...
But Java also knows the real world is messy.
Some libraries still rely on reflective final field mutation, for deserialization, proxying, or legacy injection patterns. 
JEP 500 allows teams to explicitly enable this behavior for:
- The classpath
- Specific modules
...
Now, if an illegal final mutation is discovered, meaning some code not on the exempt list of the above command-line options tries to modify a final field, the action that the JVM takes differs, depending on the setting of another command-line option:

```--enable-final-field-mutation=```
- warn (default) “Tell me when this happens.”
- deny “Fail fast. This should never happen in my code.”
- allow “I know this is unsafe, but I accept it.”
